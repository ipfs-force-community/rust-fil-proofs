...
impl<'a, Tree: 'static + MerkleTreeTrait, G: 'static + Hasher> StackedDrg<'a, Tree, G> {
    ...
    pub(crate) fn transform_and_replicate_layers_inner_fpga(
        ...
        let (tree_c_root_tx, tree_c_root_rx) = mpsc::channel();
        let (tree_r_last_root_tx, tree_r_last_root_rx) = mpsc::channel();
        let (tree_d_config_tx, tree_d_config_rx) = mpsc::channel();
        ...
        rayon::scope(|s| {
            s.spawn(move |_| {
                let tree_c_root = match layers {
                    11 => {
                        ...
                        unsafe {
                            Box::from_raw(labels_config_ptr);
                        }
                        unsafe {
                            Box::from_raw(configs_ptr);
                        }
            ...
            s.spawn(move |_| {
                ...
                let tree_r_last = measure_op(GenerateTreeRLast, || {
                    ...
                    unsafe {
                        Box::from_raw(tree_r_last_config_ptr);
                    }
                    unsafe {
                        Box::from_raw(configs_ptr);
                    }
